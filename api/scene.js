...
export default function handler(request, response) {
  const instance = typeof request.query.instance === 'string' && /^[a-zA-Z0-9-]{8,80}$/.test(request.query.instance)
    ...
    : '';
  const oembedUrl = `${APP_ORIGIN}/api/scene-oembed?instance=${encodeURIComponent(instance)}`;
  const sheetUrl = `${APP_ORIGIN}/scene.html?instance=${encodeURIComponent(instance)}`;
