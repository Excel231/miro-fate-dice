...
export default function handler(request, response) {
  ...
  const oembedUrl = `${APP_ORIGIN}/api/character-oembed?instance=${encodeURIComponent(instance)}&mode=${mode}`;
  const sheetUrl = `${APP_ORIGIN}/character.html?instance=${encodeURIComponent(instance)}`;
